...
export async function POST(request: NextRequest) {
  try {
    ...
    const { contentId, albumId, contentTitle, amount, cryptoType, txHash, userEmail, note } = body;
    ...
    const { data, error } = await supabaseAdmin
      .from("purchases")
      .insert({
        user_id: user.id, content_id: targetId, content_title: contentTitle,
        plan: "single", status: "pending", amount, crypto_type: cryptoType,
        tx_hash: txHash, note: note || null,
      })
      .select().single();
