...
export async function GET(request: NextRequest) {
  try {
    ...
    const conversationsMap = new Map<string, { userId: string; lastMessage: string; lastMessageAt: string; unread: number }>();
    for (const msg of messages || []) {
      ...
      if (!existing || msg.created_at > existing.lastMessageAt) {
        conversationsMap.set(key, {
          userId: key, lastMessage: msg.content.slice(0, 100),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: userMsg, error: userMsgError } = await supabaseAdmin
      .from("messages")
      .insert({ sender_id: user.id, receiver_id: user.id, content: content.trim(), is_ai: false })
      .select().single();
    ...
    const { data: aiMsg, error: aiMsgError } = await supabaseAdmin
      .from("messages")
      .insert({ sender_id: null, receiver_id: user.id, content: aiResult.response, is_ai: true, trigger_keyword: aiResult.triggerKeyword })
      .select().single();

    if (aiMsgError) {
      return NextResponse.json({
        userMessage: userMsg,
        aiReply: { content: aiResult.response, is_ai: true, trigger_keyword: aiResult.triggerKeyword, source: aiResult.source },
        warning: "AI reply not persisted",
      });
    }
    return NextResponse.json({ userMessage: userMsg, aiReply: aiMsg });
  } catch {
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
