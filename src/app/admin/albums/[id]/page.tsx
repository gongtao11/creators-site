...
export default function AdminAlbumDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      if (r.ok) { const d = await r.json(); const found = (d.albums || []).find((a: Album) => a.id === id); if (found) setAlbum(found); }
    } catch {}
    try {
  ...
  const uploadMore = async () => {
    ...
    setProgress(100); setUploading(false); setFiles([]); loadAll();
  };
  ...
  if (!album) return <div className="min-h-screen flex flex-col items-center justify-center gap-4"><p>Album not found</p><Link href="/admin/content" className="text-pink-500">Back</Link></div>;
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 py-8 w-full">
        ...
        {images.length === 0 ? (
          <div className="text-center py-20 bg-white dark:bg-zinc-900 rounded-2xl border">
            ...
            <p className="text-zinc-400 text-sm">Upload photos or videos</p>
          </div>
        ) : (
